...
Para auxiliar os testes da API, foram criados os endpoints de Usuários, os quais não precisam deste cabeçalho `X-User-Id`. Isso é muito útil para se testar a aplicação em um computador onde um cliente do `PostgreSQL`, como `psql` ou `pgAdmin`, não esteja instalado.
